...
function PlayerChip({
  ...
  const content = (
    <>
      ...
      <div className="min-w-0">
        <p className="truncate text-xs text-[#f0e6c8]">{player.displayName}</p>
        {player.username && (
  ...
  if (!player.username) {
    return <div className="flex min-w-0 items-center gap-2.5">{content}</div>;
  }
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-b border-[rgba(200,168,75,0.08)] py-2.5 last:border-0">
      ...
      <div className="shrink-0 text-right">
        <p className="text-xs font-bold text-[#c8a84b]">#{owner.issueNumber}</p>
    ...
    <section className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-[380px_1fr]">
        ...
        <div className="relative">
          <div
            className={`relative overflow-hidden rounded-md ${isUR ? "welcome-bonus-card" : ""}`}
          >
            {isVideo ? (
              <video
                src={card.mediaUrl}
        ...
        <div className="flex flex-col gap-6">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#c8a84b]">
              {card.rarity}
              {card.isEvent && card.eventName ? ` · ${card.eventName}` : ""}
            </span>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[
              { label: "Current Price", value: fmt(card.currentPrice) },
              { label: "Owners", value: fmt(card.ownerCount) },
              { label: "Issued", value: fmt(card.totalIssued) },
              { label: "Wishlisted", value: fmt(card.wishlistCount) },
            ].map((s) => (
              <div
                key={s.label}
        ...
        {owners.length === 0 ? (
          ...
          <>
            ...
            {owners.length > 8 && (
              ...
              >
                {showAllOwners ? "Show fewer" : `Show all ${owners.length}`}
              </button>
            )}
          </>
        )}
            ...
            <div className="flex gap-3 overflow-x-auto pb-2">
              {seriesSiblings.map((s) => (
                <Link
                  ...
                  className="w-28 shrink-0 overflow-hidden rounded-md border border-[rgba(200,168,75,0.15)] transition-colors hover:border-[#c8a84b]"
                >
                  ...
                  <p className="truncate px-1.5 py-1.5 text-[10px] text-[#f0e6c8]">
                    {s.name}
                  </p>
                </Link>
              ))}

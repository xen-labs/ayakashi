      ...
      <div className="grid grid-cols-4 gap-1.5">
        {QUICK_PERCENTS.map((p) => (
          <button
            ...
            type="button"
            disabled={max <= 0}
            onClick={() => setValue(String(Math.floor((max * p) / 100)))}
...
export default function BankVaultPage() {
  ...
  const vaultCritical =
    homeVault.owned && homeVault.health.current < homeVault.health.max * 0.3;
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="form-card border p-4">
            {txLoading ? (
              ...
              transactions.items.map((tx) => <TxRow key={tx.id} tx={tx} />)
            )}
          </div>

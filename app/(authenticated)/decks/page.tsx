...
import type { DecksResponse, DeckManageSlot, CardInstance } from "../../../lib/api";
    ...
    >
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-3 gap-2">
            {filtered.map(c => (
              ...
              >
                {c.card?.mediaUrl ? (
                  <Image src={c.card.mediaUrl} alt={c.card.name} width={100} height={140} className="h-full w-full object-cover" unoptimized />
                ) : (
                  <div className="flex h-28 items-center justify-center text-2xl">🃏</div>
                )}
                <div className="absolute bottom-0 left-0 right-0 bg-black/70 px-1 py-0.5">
                  <p className="truncate text-[8px] font-bold text-[#f0e6c8]">{c.card?.name ?? "?"}</p>
                  <p className="text-[7px] text-[rgba(200,168,75,0.50)]">{c.card?.rarity}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-[rgba(200,168,75,0.15)] px-5 py-3">
          <button type="button" disabled={page <= 1} onClick={() => { setPage(p => p - 1); load(page - 1); }}
            className="text-xs text-[rgba(200,168,75,0.60)] disabled:opacity-30 hover:text-[#c8a84b]">← Prev</button>
          <span className="text-xs text-[rgba(200,168,75,0.40)]">{page} / {totalPages}</span>
          <button type="button" disabled={page >= totalPages} onClick={() => { setPage(p => p + 1); load(page + 1); }}
            className="text-xs text-[rgba(200,168,75,0.60)] disabled:opacity-30 hover:text-[#c8a84b]">Next →</button>
        </div>
      )}
    </dialog>
  );
...
function DeckEditor({
  ...
  const saveName = async () => {
    setSavingName(true);
    try {
      await upsertDeck(slot.slotIndex, { deckName: nameInput.trim() || undefined });
      setEditing(false);
      onRefresh();
    } catch { /* noop */ }
  ...
  return (
    <div className="form-card flex flex-col gap-4 border p-5">
        ...
        {editing ? (
          ...
          <>
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => { setNameInput(slot.deckName ?? ""); setEditing(true); }}
      ...
      {pickerPos !== null && (
        <CardPickerModal onPick={handleAssign} onClose={() => setPickerPos(null)} />
      )}
    </div>
  );
}
  ...
  return (
    <section className="mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <p className="text-xs text-[rgba(200,168,75,0.45)]">
        {data.unlockedCount} / {data.maxDecks} slots unlocked · {data.deckSize} cards per deck
      </p>
      ...
      <div className="flex flex-col gap-5">
        {data.slots.map(slot => {
          ...
          if (slot.state === "empty") {
            return (
              <div key={slot.slotIndex} className="flex items-center justify-between border border-dashed border-[rgba(200,168,75,0.25)] p-5">
                <span className="text-xs uppercase tracking-widest text-[rgba(200,168,75,0.40)]">

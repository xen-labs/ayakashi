...
function ToolPill({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      disabled={!owned}
      className={`flex items-center gap-2 rounded-md border px-3 py-2 text-left text-xs transition-all disabled:cursor-not-allowed ${
        selected
    ...
    >
      <span className="flex-1 truncate font-bold">{label ?? itemId}</span>
      {!owned && (
...
function PocketKitCard({
  ...
  return (
    <div className="craft-card flex flex-col gap-3 rounded-xl p-4">
      ...
      {kit.weapon ? (
        <div
          className={`flex items-center gap-2 text-sm ${kit.weapon.owned ? "text-[#f0e6c8]" : "text-[rgba(200,168,75,0.35)]"}`}
        >
          <span className="text-lg">{kit.weapon.emoji}</span>
          <span className="font-bold">{kit.weapon.name}</span>
          {!kit.weapon.owned && (
        ...
        <p className="text-xs text-[rgba(200,168,75,0.35)]">No weapon set.</p>
      )}
...
function VaultKitCard({
  ...
  const rows: { icon: React.ReactNode; tool: LoadoutTool | null }[] = [
    { icon: <DoorOpen className="h-3.5 w-3.5" />, tool: kit.entryTool },
    { icon: <Hammer className="h-3.5 w-3.5" />, tool: kit.breachTool },
    { icon: <Wind className="h-3.5 w-3.5" />, tool: kit.escapeTool },
    { icon: <Backpack className="h-3.5 w-3.5" />, tool: kit.bag },
  ];
      ...
      <div className="flex flex-col gap-1.5">
        {rows.map(({ icon, tool }, i) =>
          tool ? (
            <div
              key={i}
              className={`flex items-center gap-2 text-xs ${tool.owned ? "text-[#f0e6c8]" : "text-[rgba(200,168,75,0.35)]"}`}
            >
              <span className="text-[rgba(200,168,75,0.45)]">{icon}</span>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            onChange={(e) =>
              setSlotNumber(
                Math.min(maxSlot, Math.max(1, Number(e.target.value))),
              )
            }
    ...
    <section className="mx-auto flex w-full max-w-4xl flex-col gap-10 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center gap-3">
          ...
          {data.loadouts.vault.length < data.maxKitsPerTier && !addingVault && (
            <button
              type="button"

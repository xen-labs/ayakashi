    ...
    <section className="relative mx-auto flex w-full max-w-5xl flex-col gap-8 px-4 py-8 pb-24 sm:px-6 lg:px-8 lg:pb-8">
      ...
      <div className="relative flex flex-col gap-1">
        <h1 className="font-display text-2xl font-black uppercase tracking-[0.05em] text-[#f0e6c8] sm:text-3xl">
          Welcome back, <span className="text-[#c8a84b]">{identity.displayName}</span>
        </h1>
        <p className="font-ui text-xs uppercase tracking-[0.14em] text-[rgba(200,168,75,0.40)]">
          @{identity.username} &nbsp;·&nbsp; Member since {formatMemberSince(identity.memberSince)}
        </p>
      ...
      <div>
        ...
        <p className="mt-3 text-center text-[10px] uppercase tracking-widest text-[rgba(200,168,75,0.30)]">
          Bank cap: {formatNumber(currency.bankCap)} &nbsp;·&nbsp; Tier {currency.bankVaultTier}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div className="form-card flex flex-col gap-4 border p-5">
          <h2 className="font-display text-xs font-bold uppercase tracking-[0.1em] text-[#c8a84b]">Progression</h2>
          <div className="flex items-center justify-between">
            <span className="text-xs uppercase tracking-widest text-[rgba(200,168,75,0.55)]">Level</span>
            <span className="text-lg font-bold text-[#e6c96a]">{progression.level}</span>
          </div>
          <div>
            ...
            <div className="h-1.5 w-full overflow-hidden bg-[rgba(200,168,75,0.10)]">
              <div className="h-full bg-[#c8a84b]" style={{ width: `${Math.min(100, (progression.xp % 1000) / 10)}%` }} />
            </div>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs uppercase tracking-widest text-[rgba(200,168,75,0.55)]">Cards Owned</span>
            <span className="text-sm font-bold text-[#f0e6c8]">{formatNumber(cardsOwned)}</span>
          </div>
        </div>

        {/* Daily claim */}
        <div className="form-card flex flex-col gap-4 border p-5">
          <h2 className="font-display text-xs font-bold uppercase tracking-[0.1em] text-[#c8a84b]">Daily Claim</h2>
          <div className="flex items-center justify-between">
            <span className="text-xs uppercase tracking-widest text-[rgba(200,168,75,0.55)]">Status</span>
            <span className={`text-xs font-bold uppercase tracking-widest ${dailyClaim.available ? "text-green-400" : "text-[rgba(200,168,75,0.50)]"}`}>
              {dailyClaim.available ? "✦ Ready" : `${formatMs(dailyClaim.remainingMs)} left`}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-xs uppercase tracking-widest text-[rgba(200,168,75,0.55)]">Streak</span>
            <span className="text-lg font-bold text-[#e6c96a]">{dailyClaim.currentStreak}d 🔥</span>
          </div>
          {!dailyClaim.streakWillContinueIfClaimedNow && !dailyClaim.available && (
        ...
        <>
          ...
          <div>
            ...
            <div className="form-card grid grid-cols-2 gap-4 border p-5 sm:grid-cols-4">
              {[
                { label: "Vault Ryo",   value: `${formatNumber(vault.ryo)} / ${formatNumber(vault.ryoCap)}` },
                { label: "Vault Kitsu", value: `${formatNumber(vault.kitsu)} / ${formatNumber(vault.kitsuCap)}` },
                { label: "Health",      value: `${vault.health} / ${vault.maxHealth}` },
                { label: "Tier",        value: String(vault.tier) },
              ].map((s) => (
                <div key={s.label} className="flex flex-col items-center gap-1 text-center">
                  <span className="text-sm font-bold text-[#e6c96a]">{s.value}</span>
            ...
            {vault.repairCost && (
              <p className="mt-2 text-center text-xs text-[rgba(200,168,75,0.45)]">
                Repair: {formatNumber(vault.repairCost.ryo)} ryo + {vault.repairCost.materialQty}× {vault.repairCost.material}
              </p>
            )}
          </div>
        </>
          ...
          <div className="form-card border p-5">
            ...
            <div className="flex flex-col gap-2">
              {pendingFriendRequests.requests.map((r) => (
                <div key={r.jid} className="flex items-center justify-between gap-3">
                  <div>
                    <span className="text-sm text-[#f0e6c8]">{r.displayName}</span>
                    {r.username && <span className="ml-1.5 text-xs text-[rgba(200,168,75,0.45)]">@{r.username}</span>}
                  </div>
                </div>
              ))}
            </div>

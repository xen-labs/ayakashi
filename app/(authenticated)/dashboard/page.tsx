...
export default function Dashboard() {
    ...
    const loadTrades = useCallback(async () => {
        try {
            ...
            setPendingTrades(
                res.trades.filter(t => t.recipient.username === me)
            );
        } catch {
            /* noop */
        }
    ...
    const totalNotifications =
        pendingFriendRequests.count + pendingTrades.length;
        ...
        <section className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-col gap-1">
                <h1 className="font-display text-2xl font-bold text-[#f0e6c8] sm:text-3xl">
                    Welcome back,{" "}
                    <span className="text-ayakashi-gold">
            ...
            <div className="grid grid-cols-2 gap-3 [animation:shop-card-in_0.35s_ease-out_backwards] sm:gap-4">
                <CurrencyChip
                    type="ryo"
                    label="Ryo"
                    value={fmt(currency.ryo)}
                />
                <CurrencyChip
                    type="kitsu"
                    label="Kitsu"
                    value={fmt(currency.kitsu)}
                />
            </div>
                ...
                {vault ? (
                    ...
                    >
                        ...
                        <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                            ...
                            <span className="text-[10px] uppercase tracking-widest text-[rgba(200,168,75,0.45)]">
                                Home Vault Tier {vault.tier} · {vault.health}/
                                {vault.maxHealth} HP
                            </span>
                        </div>
                        {vault.repairCost && (

...
function ListingTile({
    ...
    // See CardTile.tsx for the full reasoning — same card-back
    // placeholder pattern, kept local to this tile for the same reason
    // the rest of ListingTile is local (different data shape than the
    // catalog browser's CardTile).
            ...
            >
                <div className="relative aspect-[3/4] w-full">
                    {card ? (
                        <>
                            {/* Loading placeholder — see CardTile.tsx for
                                full reasoning. Sits under the real art,
                                fades out once it's loaded. */}
                            <img
                    ...
                    {/* z-[2] on everything below — sits above BOTH art
                        ...
                        regardless of which is currently visible. Previously
                        unset, which put these at the same stacking level as
                        the placeholder and let the real art layer cover
                        them once loaded. */}
                    ...
                    <span className="absolute right-1.5 top-1.5 z-[2] rounded-sm bg-black/70 px-1.5 py-0.5 text-[9px] font-bold text-[rgba(200,168,75,0.55)]">
                        #{listing.issueNumber}
                    </span>
...
// ── Buy confirm / listing detail — [REMOVED, this pass] ─────────────
// The whole BuyModal (detail view + buy flow) moved to a dedicated
// route: app/marketplace/[instanceId]/page.tsx. Reasons: (1) a real
// shareable URL + working back button, which a modal can't offer; (2)
// this page's grid tile click now navigates there via router.push
// instead of opening local state. OwnerHistoryRow/METHOD_LABELS/
// fmtDate/BuyPhase all moved with it (duplicated in the new file
// rather than shared, since this is now a separate route with its own
// bundle — matches this codebase's existing pattern of not sharing
// small page-local helpers across route boundaries).
                            ...
                            <div className="flex items-center justify-center gap-3 pt-2">
                                ...
                                <div className="flex items-center gap-1.5 text-xs text-[rgba(200,168,75,0.55)]">
                                    <input
                                        type="number"
                                        min={1}
                                        max={myListingsTotalPages}
                                        value={myListingsPage}
                                        onChange={e => {
                                            const p = Math.min(
                                        ...
                                        className="form-input h-8 w-14 border px-1.5 text-center text-xs outline-none"
                                    />
                                    <span>/ {myListingsTotalPages}</span>
                                </div>
                                <button
                                    type="button"
                                    disabled={

...
function ListCardModal({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError("");
    try {
      await listCardForAuction(selected.instanceId, parsedBuyNow);
      onListed();
    } catch (err) {
              ...
              <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                {cards.map((c) => {
                  ...
                  const disabled = Boolean(
                    c.isLocked || c.pendingTradeId || c.activeLoanId,
                  );
          ...
          <>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-[10px] text-[rgba(200,168,75,0.4)]">
                Starting bid, bid increment, and auction duration are set
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await getAuctions({
        page,
        sort,
        rarity: rarityFilter.size ? [...rarityFilter].join(",") : undefined,
      });
    ...
    <section className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-4 py-14 text-center">
          <p className="text-sm text-[rgba(200,168,75,0.5)]">{loadError}</p>
          <button
            type="button"
            onClick={() => load()}
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
            {listings.map((listing, i) => (
              <AuctionTile
                ...
                listing={listing}
                index={i}
                canAfford={kitsu === null || kitsu >= listing.currentBid}
                onOpen={(l) => router.push(`/auctions/${l.instanceId}`)}
              />
            ))}
          </div>

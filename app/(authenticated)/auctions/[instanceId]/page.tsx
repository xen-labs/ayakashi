...
function useCountdown(expiresAt: string | undefined) {
  ...
  return { label, urgent: remainingMs < 5 * 60_000, done: false, remainingMs };
}
...
function BidRow({
  ...
  return (
    <div
      ...
      style={{
        animationDelay: isLatest ? "0ms" : `${Math.min(index, 8) * 25}ms`,
      }}
    >
...
export default function AuctionDetailPage() {
  ...
  useEffect(() => {
    ...
    es.addEventListener("bid", (e) => {
      ...
      setDetail((prev) => {
        ...
        const gotOutbid = wasMe && data.bidderJid !== prev.highestBidderId;
        if (gotOutbid) {
          setOutbidToast(true);
  ...
  return (
    <section className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-8 pb-[calc(var(--bottom-nav-h,64px)+11rem)] sm:px-6 lg:px-8 lg:pb-8">
      <button
        type="button"
        onClick={() => router.push("/auctions")}
        ...
        <div className="fixed left-1/2 top-4 z-50 -translate-x-1/2 [animation:shop-toast-in_0.3s_ease-out]">
          <div className="auction-outbid-shake flex items-center gap-2 rounded-sm border border-red-500/40 bg-[#1a0500]/95 px-4 py-2.5 shadow-[0_8px_32px_rgba(0,0,0,0.6)]">
            ...
            <p className="text-xs font-semibold text-red-300">
              ...
              <span className="text-red-200">{detail.highestBidderName}</span>
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-5 sm:flex-row">
            {/* Card art */}
            <div
              className={`relative mx-auto h-64 w-48 shrink-0 overflow-hidden rounded-md border bg-black transition-shadow sm:mx-0 ${rarityClass} ${rarityShadow}`}
            >
              {card ? (
                isVideo ? (
                  <video
                ...
                <div className="flex h-full w-full items-center justify-center text-[rgba(200,168,75,0.25)]">
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-2">
              ...
              >
                ...
                {countdown.done ? "Auction Ended" : countdown.label}
              </div>
            </div>
          </div>
              ...
              {ended.reason === "lapsed_no_bids" ? (
                ...
                <>
                  ...
                  <p className="text-sm font-bold text-red-300">
                    Something went wrong settling this auction
                  </p>
                  <p className="text-xs text-[rgba(200,168,75,0.45)]">
                    The seller has been notified — please check back.
                  </p>
                </>
              ) : (
                <>
      ...
      {canBid && (
        ...
        >
          ...
          {bidError && (
            <p className="mt-1.5 text-[10px] text-red-400">{bidError}</p>
          )}
        </div>
      )}
      ...
      {buyNowConfirm && detail.buyNowPrice !== null && (
        <BuyNowModal
          ...
          onConfirm={() => handlePlaceBid(detail.buyNowPrice as number)}
          placing={placingBid}
        />
      )}
    </section>
        ...
        <>
          <div className="flex gap-1.5">
            {quickAdds.map((amt, i) => (
              ...
              >
                {i === 0 ? "Min" : `+${formatNumber(amt - minBid)}`}
              </button>
            ))}
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-4 px-5 py-5">
        ...
        >
          {placing ? "Confirming…" : `Buy Now for ${formatNumber(price)} Kitsu`}
        </button>
      </div>
    </dialog>
  );
    ...
    <div className="form-card flex h-80 flex-col rounded-md border">
          ...
          chat.map((m, i) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="break-words text-xs text-[#f0e6c8]">{m.text}</p>
              </div>
            </div>
          ))
        )}
      ...
      <div className="flex items-center gap-2 border-t border-[rgba(200,168,75,0.12)] px-3 py-2.5">
        <input
          ...
          className="form-input h-9 flex-1 border px-3 text-xs outline-none"
        />
        <button
          type="button"

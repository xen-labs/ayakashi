...
// Upgrade and Fusion moved out (same "manage what you've built/own"
// bucket as Craft/Bank-Vault on mobile) — reachable from the Profile
// dropdown, not top-level rail items.
//
...
export function TopBar({ user }: { user: MeResponse | null }) {
    ...
    return (
        <header className="topbar sticky top-0 z-30 flex items-center justify-between px-4 py-3 sm:px-6">
            ...
            <div className="flex items-center gap-5">
                <Link href="/" className="flex items-center">
                    <Image
                        ...
                        width={30}
                        height={30}
                        className="brand-mark-glow h-auto w-[30px]"
                        unoptimized
                    />
                </Link>
            ...
            {/* ── Right: Coins + Profile (auth) or Login (guest) ── */}
            <div className="flex items-center gap-4">
                {isAuthenticated ? (
                    <>
                        {/* separates the brand cluster from currency at widths
                where the nav links are hidden (mobile) */}
                        <span
                            className="topbar-brand-divider h-6 w-px shrink-0"
                            aria-hidden="true"
                        />
                        <div className="flex items-center gap-3">
                            ...
                            <div
                                className="topbar-divider h-5 w-px"
                                aria-hidden="true"
                            />
                        </div>
                    ...
                    // [NEW] Guest state — homepage is now shared between logged-in
                    // and logged-out visitors (same layout, personal greeting vs.
                    // CTA swapped further down the page), so TopBar needs a real
                    // guest affordance instead of rendering nothing but the logo.
                    <Link
                        href="/login"
                        className="text-xs font-bold uppercase tracking-[0.16em] text-[rgba(200,168,75,0.75)] transition-colors hover:text-[#c8a84b]"
                    >
                        Log in
                    </Link>
                )}

...
export function PlayerSearch({ compact = false, onSelect, placeholder = "Search players…" }: Props) {
  ...
  const doSearch = async (q: string, p: number) => {
    ...
    try {
      ...
      if (err instanceof ApiResponseError && err.error.code === "query_too_short") {
        setResults([]);
      } else {
  ...
  const clear = () => { setQuery(""); setResults([]); setError(""); setPage(1); setTotalPages(1); };
  ...
  const ResultRow = ({ r }: { r: PlayerSearchResult }) => {
    const inner = (
      <div className="flex items-center gap-3 border-b border-[rgba(200,168,75,0.08)] px-3 py-2.5 last:border-0 transition-colors hover:bg-[rgba(200,168,75,0.05)]">
        <div className="h-8 w-8 shrink-0 overflow-hidden border border-[rgba(200,168,75,0.20)] bg-[rgba(200,168,75,0.05)]">
          {r.avatarUrl ? (
            <Image src={r.avatarUrl} alt={r.displayName} width={32} height={32} className="h-full w-full object-cover" unoptimized />
          ) : (
        ...
        <div className="flex min-w-0 flex-col">
          <span className="truncate text-sm font-bold text-[#f0e6c8]">{r.displayName}</span>
          <span className="text-[10px] text-[rgba(200,168,75,0.40)]">@{r.username}</span>
        </div>
      </div>
    );
    ...
    if (onSelect) {
      return (
        <button type="button" className="w-full text-left" onClick={() => onSelect(r)}>
          {inner}
        </button>
      );
    }
  ...
  const showResults = results.length > 0 || loading || (error !== "") || query.length >= 2;
    ...
    return (
      <div className="relative w-full">
        <div className="flex items-center gap-2 border border-[rgba(200,168,75,0.25)] bg-black/40 px-3">
          ...
          />
          {query && (
            <button type="button" onClick={clear} className="text-[rgba(200,168,75,0.40)] hover:text-[#c8a84b]">
        ...
        {showResults && (
          <div className="absolute left-0 right-0 top-full z-30 border border-[rgba(200,168,75,0.25)] border-t-0 bg-[#0d0c00] shadow-xl">
            {loading && <p className="px-4 py-3 text-xs text-[rgba(200,168,75,0.40)]">Searching…</p>}
            {error && <p className="px-4 py-3 text-xs text-red-400">{error}</p>}
            {!loading && results.length === 0 && query.length >= 2 && !error && (
              <p className="px-4 py-3 text-xs text-[rgba(200,168,75,0.40)]">No players found.</p>
            )}
            {results.map(r => <ResultRow key={r.username} r={r} />)}
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!loading && query.length >= 2 && results.length === 0 && !error && (
        <p className="text-sm text-[rgba(200,168,75,0.40)]">No players found for &quot;{query}&quot;.</p>
      )}
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-xs text-[rgba(200,168,75,0.40)]">Page {page} / {totalPages}</span>
              <button
                type="button"

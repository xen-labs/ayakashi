...
import { X, Coins, Sparkles, ArrowLeft } from "lucide-react";
import {
  getMaterialShop,
...
export default function Shop() {
  ...
  const loadAll = useCallback(async () => {
    try {
      const [shopRes, purchasesRes, robRes, dashRes] = await Promise.all([
        getMaterialShop(),
        getMyMaterialPurchases(),
        getRobItemShop(),
        getDashboard(),
  ...
  return (
    <main className="relative flex min-h-dvh flex-col overflow-hidden bg-theme-texture bg-cover bg-center px-4 py-8 sm:px-6 lg:px-8">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6">
        {/* Header */}
        <header className="flex items-center justify-between">
          <Link
            href="/dashboard"
            className="flex items-center gap-2 text-sm font-semibold uppercase tracking-widest text-astral-gold transition-colors hover:text-white"
          >
        ...
        {tab === "rob-items" && robItems && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {robItems.map((item) => (
              <div
                key={item.itemId}
                className="form-card flex flex-col gap-3 border p-4 shadow-[0_0_20px_rgba(0,0,0,0.25)] backdrop-blur-md"
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">{item.emoji}</span>
                ...
                <button
                  ...
                  className="mt-1 h-9 border border-astral-gold text-xs font-bold uppercase tracking-widest text-astral-gold transition-colors hover:bg-astral-gold hover:text-black"
                >
                  Buy
                </button>
              </div>

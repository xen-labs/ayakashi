...
export default function Home() {
  ...
  // Fade stats out as the user scrolls past the first screen
  useEffect(() => {
    const onScroll = () => {
      const scrollY = window.scrollY;
      const viewH = window.innerHeight;
      // Fully visible at top, fully gone by 40% of one viewport scrolled
  ...
  return (
    <>
      ...
      <footer className="fixed bottom-0 left-0 right-0 z-50 team-footer border-t px-4 py-2">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="team-member-link flex items-center gap-1 text-xs transition-opacity hover:opacity-70"
            >
      ...
      <main className="relative bg-theme-texture bg-cover bg-center bg-no-repeat">
        {/* Fixed overlay + glow — stays behind all content */}
        ...
        <section className="relative z-10 flex min-h-dvh flex-col items-center justify-between px-4 pb-16 pt-10 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-1 items-center justify-center w-full">
            <div
              className={`flex w-full flex-col items-center text-center transition-all duration-500 ${
                loggedIn ? "max-w-2xl" : "max-w-4xl"
              }`}
            >
              <Image
                ...
                className={`logo-filter mb-6 h-auto transition-all duration-500 ${
                  loggedIn ? "w-20 sm:w-24" : "w-28 sm:w-36 lg:w-44"
                }`}
                priority
                unoptimized
              />
              ...
              <h1
                className={`theme-heading mb-4 font-bold uppercase tracking-widest transition-all duration-500 ${
                  loggedIn
              ...
              <p
                className={`theme-body mb-8 max-w-2xl leading-7 sm:mb-10 transition-all duration-500 ${
        ...
        {/* ── Screen 2: placeholder content ───────────────────────── */}
        <section className="relative z-10 min-h-dvh px-4 py-20 pb-24 sm:px-6 lg:px-8 flex items-center justify-center">
